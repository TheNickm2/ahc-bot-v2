 ...
 */
export async function doesRedisValueExist(key: string) {
  try {
 ...
 * @return {(true | undefined)} true if successful, undefined (falsy) if an error occurred while setting the value.
 */
export async function setRedisHashTable(key: string, value: object) {
  try {
 ...
 */
export async function getRedisHashValue(key: string, property: string) {
  try {
 ...
 */
export async function getRedisHashTable(key: string) {
  try {
